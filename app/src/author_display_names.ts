...
export async function hydrateAuthorDisplayNames():Promise<void>{
 ...
 pending=(async()=>{
  ...
  for(let page=0;page<100;page++){
   const response=await fetch(`/api/library/author-names?page=${page}&limit=100`,{credentials:'same-origin',cache:'no-cache',redirect:'error',signal})
   ...
   try{for(;;){const part=await reader.read();if(part.done)break;size+=part.value.length;if(size>262144)throw Error('author_names_oversized');chunks.push(part.value)}}catch(error){await reader.cancel();throw error}finally{reader.releaseLock()}
   const bytes=new Uint8Array(size);let offset=0;for(const chunk of chunks){bytes.set(chunk,offset);offset+=chunk.length}
   const value=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(bytes))
   if(value.schemaVersion!==1||value.page!==page||typeof value.hasMore!=='boolean'||typeof value.version!=='string'||!/^\d+:\d+:\d+$/.test(value.version)||!Array.isArray(value.names)||value.names.length>100||version&&version!==value.version)throw Error('author_names_invalid')
   ...
   if(!value.hasMore){if(ticket===generation){names=next;hasSnapshot=true;expires=Date.now()+30000;notifyNames()}return}
   if(value.names.length!==100)throw Error('author_names_invalid')
  }
