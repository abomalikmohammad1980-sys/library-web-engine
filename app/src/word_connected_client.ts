...
async function request(path:string,init:RequestInit={},timeout=10000,signal?:AbortSignal):Promise<Response>{
 const token=connectionToken
 if(!token)throw Error('اربط مساعد Word من نافذة الرفع أولًا.')
 try{
  const response=await fetch(origin+path,{...init,headers:{...init.headers,'X-Khizana-Token':token},mode:'cors',credentials:'omit',cache:'no-store',redirect:'error',signal:signal?AbortSignal.any([signal,AbortSignal.timeout(timeout)]):AbortSignal.timeout(timeout)})
...
export async function connectWord(token:string,signal?:AbortSignal):Promise<void>{
 if(!/^[a-f0-9]{64}$/.test(token.trim()))throw Error('انسخ رمز الاتصال الكامل من نافذة مساعد Word.')
 connectionToken=token.trim()
 try{const response=await request('/v1/status',{},10000,signal);const value=await response.json();if(!response.ok||value.contract!=='khizana-connected-word/1'||value.ready!==true)throw Error('إصدار المساعد غير متوافق.')}
