...
export async function convertWithConnectedWord(file:File,onProgress:()=>void=()=>{}):Promise<Uint8Array>{
 ...
 while(Date.now()<deadline){
  ...
  onProgress();await new Promise(resolve=>setTimeout(resolve,1000))
  ...
  if(Number(result.headers.get('Content-Length'))>128*1024*1024)throw Error('حزمة Word أكبر من الحد المسموح.')
  const reader=result.body?.getReader();if(!reader)throw Error('حزمة Word فارغة.')
  const chunks:Uint8Array[]=[];let size=0
  while(true){const part=await reader.read();if(part.done)break;size+=part.value.length;if(size>128*1024*1024){await reader.cancel();throw Error('حزمة Word أكبر من الحد المسموح.')};chunks.push(part.value)}
