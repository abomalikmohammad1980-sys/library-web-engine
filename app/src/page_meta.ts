...
import {pageMetaFor,type PublicSeoRecord} from './page_meta_model'
...
export function bindPageMeta(content:HTMLElement,isCurrent:()=>boolean):()=>void{
 ...
 let meta=pageMetaFor(path),last=''
 const apply=()=>{if(isCurrent()&&!controller.signal.aborted){const key=JSON.stringify(meta);if(key!==last||document.title!==meta.title){setPageMeta(meta);last=key}}}
 apply()
 ...
 if(match){const id=match[1]==='authors'?match[2]!.padStart(6,'0'):match[2]!;void fetch(`/api/seo/record?kind=${match[1]}&id=${id}`,{signal:controller.signal,cache:'no-store'}).then(async response=>{
  if(!response.ok)throw Error('seo_meta_unavailable');const data=await response.json() as {record?:PublicSeoRecord};if(data.record)meta=pageMetaFor(path,data.record);apply()
 ...
 const observer=new MutationObserver(apply);observer.observe(content,{subtree:true,childList:true,characterData:true})
