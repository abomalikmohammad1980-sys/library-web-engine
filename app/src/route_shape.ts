...
export function validRouteShape(parts:readonly string[]):boolean{
 ...
 if(parts.length===1&&['recommendations','categories'].includes(parts[0]!))return true
 if(parts.length===2&&parts[0]==='categories'){try{return Boolean(decodeURIComponent(parts[1]!).trim())&&!/[\/\\]/.test(decodeURIComponent(parts[1]!))}catch{return false}}
