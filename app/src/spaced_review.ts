...
export function recordReview(annotationId: string, outcome: 'remembered' | 'again', now = Date.now()): SpacedReviewRecord {
  ...
  localStorage.setItem(KEY, JSON.stringify([record, ...records.filter(item => item.annotationId !== annotationId)]))
  return record
}
...
export function saveSpacedReviews(records: readonly SpacedReviewRecord[]): void {
  ...
  if (valid.length) localStorage.setItem(KEY, JSON.stringify(valid)); else localStorage.removeItem(KEY)
}
