...
export function createWordResumeRunner(options:{scope:()=>string;run:(scope:string)=>Promise<void>}){
 let requested=false,flight:Promise<void>|undefined;
 return ():Promise<void>=>{requested=true;return flight??=(async()=>{while(requested){requested=false;const owner=options.scope();await options.run(owner);if(options.scope()!==owner)requested=true}})().finally(()=>{flight=undefined})};
}
...
export async function resumeWordCheckpoint<T extends{id:string;title:string;originalSha256?:string;fileSize?:number}>(options:{storage:Store;scope:()=>string;getBook:(id:string)=>Promise<T|undefined>;isWord:(book:T)=>boolean;ready:(book:T)=>Promise<boolean>;fingerprint?:(book:T)=>Promise<string>;enqueue:(id:string,title:string,scope:string,revision:string)=>void;yield?:()=>Promise<void>}):Promise<number>{
 const owner=options.scope(),ids=pendingWordIds(options.storage,owner);let queued=0;
 for(const id of ids){if(options.scope()!==owner)break;await(options.yield?.()??new Promise<void>(r=>setTimeout(r,0)));if(options.scope()!==owner)break;const book=await options.getBook(id);if(options.scope()!==owner)break;if(book&&options.isWord(book)){const ready=await options.ready(book);if(options.scope()!==owner)break;if(!ready){const revision=options.fingerprint?await options.fingerprint(book):book.originalSha256||`${book.id}:${book.fileSize}`;if(options.scope()!==owner)break;options.enqueue(id,book.title,owner,revision);queued++;continue}}
