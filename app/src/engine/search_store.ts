...
async function searchExistingBooks(query: string, options: SearchQueryOptions = {}): Promise<SearchResultSet> {
  ...
  if(fields.has('body')){
    ...
    const grouped=new Map<string,{hit:(typeof page.hits)[number];count:number}>();for(const hit of page.hits){const current=grouped.get(hit.id);current?current.count++:grouped.set(hit.id,{hit,count:1})}
    ...
    const result=[...localResults,...fastResults.map(hit=>({...hit,sourceKind:'shamela4.1' as const}))] as SearchResultSet;result.totalOccurrences=page.total+localOccurrences;result.unavailableBookIds=[...(page.unavailableBookIds??[]),...scopedFailedIds];result.unopenedBookIds=unopened;result.pendingBookIds=localFederatedReady||localIndexFailed?page.pendingBookIds:[...(page.pendingBookIds??[]),'local-formats'];result.localIndexFailed=scopedIndexFailed;result.coverageComplete=page.coverageComplete&&metadataScope.coverageComplete&&localFederatedReady&&!scopedIndexFailed&&unopened.length===0;sortSearchResultsByDeath(result);return result
  }
