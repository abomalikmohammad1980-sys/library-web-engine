...
const fetch=((input:RequestInfo|URL,init?:RequestInit)=>{const url=String(input),match=/^\.\/library\/shamela\/batches\/(batch-\d{4})\/books\/([^/?#]+\.json)$/u.exec(url);return match?fetchPagesDataAsset('corpus',`${match[1]}/books/${match[2]}`,url,globalThis.fetch):globalThis.fetch(input,init)}) as typeof globalThis.fetch
...
async function hydrateEntry(located:LocatedShamelaPackBook):Promise<StoredBook>{
 ...
 try{
  ...
  let parsed:PackBook
  try{parsed=JSON.parse(new TextDecoder().decode(packed)) as PackBook}catch(error){throw new ShamelaPackSeedError('shamela_pack_book_json_invalid',error)}
  ...
  const book=materializeShamelaPackBook(parsed,packed,entry.sha256,entry.catalog.title)
