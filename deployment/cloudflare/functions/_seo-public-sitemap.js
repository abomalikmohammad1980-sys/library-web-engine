import {indexedSitemap,indexedSitemapPages} from './_seo-indexed-sitemap.js'
const PUBLIC="b.visibility='public' AND b.review_status='approved' AND b.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM central_book_overrides o WHERE o.book_id IN (b.id,'central-submission:'||b.id,'account-book:'||b.id) AND (o.visibility<>'public' OR o.logically_deleted_at IS NOT NULL))"
const xml=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&apos;'}[c]))
export async function publicSitemap(db,url,options={}){
 if(options.indexedOnly)return indexedSitemap(db,url,options)
...
export async function publicSitemapPages(db,options={}){
 if(options.indexedOnly)return indexedSitemapPages(db)
