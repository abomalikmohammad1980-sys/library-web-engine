...
export async function onRequestPatch(context){
  ...
  const [updated]=await context.env.VISITORS_DB.batch([upsert,audit]);if(updated?.results?.[0]?.book_id!==bookId){const latest=await context.env.VISITORS_DB.prepare('SELECT revision,visibility,logically_deleted_at FROM central_book_overrides WHERE book_id=?1').bind(bookId).first();const confirmed=deletedReply(latest);if(confirmed)return confirmed;return json({error:'central_book_conflict',revision:Number(latest?.revision??currentVersion)},409)}
  return afterPublicBookMutation(context,json({id:bookId,action,visibility,logicallyDeleted:action==='delete',revision:nextVersion}))
}
