...
import {validJpegSource} from '../_jpeg-source.js'
const MAX_BYTES=64*1024*1024
export async function boundedForm(request){
...
const BOOK_TYPES=new Map([
  ...
  ['.epub','application/epub+zip'],['.bok','application/octet-stream'],['.txt','text/plain; charset=utf-8'],['.md','text/markdown; charset=utf-8'],['.html','text/html; charset=utf-8'],['.htm','text/html; charset=utf-8'],
])
...
async function validatedBookType(file){const extension=extensionOf(file.name),mime=BOOK_TYPES.get(extension);if(!mime)return null;const bytes=new Uint8Array(await file.slice(0,512).arrayBuffer())
  if(mime==='image/jpeg'&&!await validJpegSource(file))return null
  ...
  if(extension==='.html'||extension==='.htm'){
    if(file.size>20*1024*1024||!/<!doctype\s+html\b|<html\b|<body\b/i.test(new TextDecoder().decode(bytes)))return null
    const reader=file.stream().getReader()
    try{const decoder=new TextDecoder('utf-8',{fatal:true});let seen=false;for(;;){const {done,value}=await reader.read();if(done)break;const text=decoder.decode(value,{stream:true});if(text.includes('\0'))return null;seen||=Boolean(text.trim())}decoder.decode();if(!seen)return null}catch{return null}finally{await reader.cancel().catch(()=>{})}
  }
...
export async function reserveQuota(env,subject,bytes){const maxBytes=positiveLimit(env.ACCOUNT_MAX_STORAGE_BYTES,DEFAULT_ACCOUNT_BYTES),maxBooks=positiveLimit(env.ACCOUNT_MAX_BOOKS,DEFAULT_ACCOUNT_BOOKS),create=env.VISITORS_DB.prepare('INSERT OR IGNORE INTO account_storage_usage(subject) VALUES(?1)').bind(subject),reserve=env.VISITORS_DB.prepare('UPDATE account_storage_usage SET used_bytes=used_bytes+?1,book_count=book_count+1,updated_at=CURRENT_TIMESTAMP WHERE subject=?2 AND used_bytes+?1<=?3 AND book_count<?4').bind(bytes,subject,maxBytes,maxBooks),results=await env.VISITORS_DB.batch([create,reserve]);return Number(results[1]?.meta?.changes??0)===1}
export async function releaseQuota(env,subject,bytes){await env.VISITORS_DB.prepare('UPDATE account_storage_usage SET used_bytes=MAX(0,used_bytes-?1),book_count=MAX(0,book_count-1),updated_at=CURRENT_TIMESTAMP WHERE subject=?2').bind(bytes,subject).run()}
...
export async function createBookSubmission(context,{publishNew=false}={}){
  ...
  let extras={metadata:null,assets:[],htmlResources:[]}
  ...
  const totalBytes=file.size+extras.assets.reduce((n,a)=>n+a.file.size,0)+extras.htmlResources.reduce((n,a)=>n+a.file.size,0)
  if(mime==='image/jpeg'&&(!extras.assets.some(a=>a.kind==='pdf')||extras.assets.some(a=>a.kind==='volume'&&a.mime!=='image/jpeg')))return json({error:'invalid_book_metadata'},400)
  ...
  let sourceKey=await sourceId(account.subject,file)
  if(mime==='image/jpeg'){
   const pages=[]
   for(const asset of extras.assets.filter(a=>a.kind==='volume'))pages.push([asset.partNumber,await assetDigest(asset.file)])
   const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify([sourceKey,pages])))
   sourceKey='src-'+[...new Uint8Array(digest)].map(b=>b.toString(16).padStart(2,'0')).join('')
  }
  if(extras.htmlResources.length){
   const members=[]
   for(const resource of extras.htmlResources)members.push([resource.path,resource.file.size,await assetDigest(resource.file)])
   members.sort((a,b)=>a[0].localeCompare(b[0]))
   const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify([sourceKey,members])))
   sourceKey='src-'+[...new Uint8Array(digest)].map(b=>b.toString(16).padStart(2,'0')).join('')
  }
  ...
  if(existing?.id===id){
   if((extras.metadata||extras.assets.length||extras.htmlResources.length)&&!existing.deletedAt){
    ...
    if(extras.htmlResources.length){const resources=await context.env.VISITORS_DB.prepare('SELECT relative_path AS path,byte_length AS bytes,sha256 FROM user_book_html_resources WHERE book_id=?1').bind(id).all();if((resources.results??[]).length!==extras.htmlResources.length)return json({error:'existing_book_metadata_conflict'},409);for(const resource of extras.htmlResources){const hash=await assetDigest(resource.file);if(!(resources.results??[]).some(row=>row.path===resource.path&&row.bytes===resource.file.size&&row.sha256===hash))return json({error:'existing_book_metadata_conflict'},409)}}
  ...
  try{uploaded.push(key);await context.env.LIBRARY_R2.put(key,file.stream(),{httpMetadata:{contentType:mime},customMetadata:{owner:account.subject,bookId:id}})
   ...
   for(const resource of extras.htmlResources){resource.id=crypto.randomUUID();resource.key=`private/${account.subject}/${id}/${resource.id}/${safeFileName(resource.file.name)}`;resource.sha=await assetDigest(resource.file);uploaded.push(resource.key);await context.env.LIBRARY_R2.put(resource.key,resource.file.stream(),{httpMetadata:{contentType:resource.mime},customMetadata:{owner:account.subject,bookId:id}})}
  ...
  try{
    if(publishNew){
      ...
      if(extras.metadata||extras.assets.length||extras.htmlResources.length){
       ...
       for(const resource of extras.htmlResources)statements.push(htmlResourceStatement(db,id,resource,key))
      }
      ;[inserted]=await db.batch(statements)
    }else if(extras.metadata||extras.assets.length||extras.htmlResources.length){
      const db=context.env.VISITORS_DB
      ...
      ;[inserted]=await db.batch([create,metadata,...extras.assets.map(a=>assetStatement(db,id,a,key,extras.wordBundle)),...extras.htmlResources.map(resource=>htmlResourceStatement(db,id,resource,key))])
...
function htmlResourceStatement(db,id,resource,key){
 return db.prepare('INSERT INTO user_book_html_resources(resource_id,book_id,relative_path,object_key,mime_type,byte_length,sha256) SELECT ?1,?2,?3,?4,?5,?6,?7 WHERE EXISTS(SELECT 1 FROM user_books WHERE id=?2 AND object_key=?8)').bind(resource.id,id,resource.path,resource.key,resource.mime,resource.file.size,resource.sha,key)
}
