...
export async function onRequestDelete(context){
  ...
  if(!row.deleted_at){
    ...
    const marked=await context.env.VISITORS_DB.prepare("UPDATE user_books SET deleted_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?1 AND owner_subject=?2 AND deleted_at IS NULL AND NOT(visibility='public' AND review_status='approved') RETURNING id").bind(bookId,account.subject).first()
    if(marked?.id!==bookId)return json({error:'published_book_must_be_withdrawn'},409)
  }
  ...
  return afterPublicBookMutation(context,json({id:bookId,deleted:true}))
}
export async function onRequestPatch(context){
  ...
  if(result[0]?.results?.[0]?.id!==id)return json({error:'account_review_conflict'},409)
  return afterPublicBookMutation(context,json({id,title:body.title.trim(),author:body.author.trim(),category:body.category.trim(),reviewVersion:body.reviewVersion+1,reviewStatus:'pending'}))
}
