...
export async function queryPublicBooks(db,params){
 ...
 let where=`WHERE public_book_search_fts MATCH ?1 AND instr(r.normalized,?2)>0 AND (r.field=?3 ${params.get('pdfBookmarks')==='0'?'':"OR (?3='body' AND r.field='heading' AND j.coverage_mode='pdf-bookmarks-only')"})`
...
export async function onRequest({request,env}){
 ...
 try{return await queryPublicBooks(env.VISITORS_DB,new URL(request.url).searchParams)}catch{return json({error:'search_unavailable'},503)}
}
