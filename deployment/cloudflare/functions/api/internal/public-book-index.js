...
async function leased(db,lease,now){
...
export function validateExtraction(value,mime){
 ...
 return {coverageMode:value.coverageMode,rows:value.rows.map(r=>clean(r,false)),headings:value.headings.map(r=>clean(r,true))}
}
export async function onRequest(context){
 ...
 try{
  ...
  if(input.op==='claim'){
   step='claim'
   if(Object.keys(input).length!==1)return json({error:'invalid_operation'},400)
   const job=await claimPublicBookIndex(db,{token:crypto.randomUUID(),now:now(),leaseSeconds:300})
   ...
   return json({job:{book_id:job.book_id,generation:job.generation,lease_token:job.lease_token,mime:job.source.mime_type}})
  }
  step='lease'
  const job=await leased(db,input.lease,now());if(!job)return json({error:'lease_expired'},409)
  if(input.op==='fail'){
   ...
   return json({ok:await failPublicBookIndex(db,job,now(),input.code)})
  }
  step='source'
  const bytes=await sourceBytes(bucket,job),sourceSha256=await sha(bytes)
  if(input.op==='source'){
   if(!await leased(db,input.lease,now()))return json({error:'lease_expired'},409)
  ...
  step='validate'
  const extracted=validateExtraction(input.extracted,job.mime_type)
  const artifact=encoder.encode(JSON.stringify({contract:'public-book-index/1',bookId:job.book_id,generation:job.generation,sourceSha256,parserVersion:'bounded-account-v1',title:job.title,author:job.author,...extracted}))
  ...
  step='search'
  const staged=await preparePublicBookSearch(db,job,JSON.parse(new TextDecoder().decode(artifact)),manifestSha256,now())
  if(staged==='pending'){
  ...
  step='activate'
  const ok=await activatePublicBookIndex(db,job,now(),{manifestSha256,sourceSha256,artifactKey,parserVersion:'bounded-account-v1',coverageMode:extracted.coverageMode,complete:true,checkpoint})
