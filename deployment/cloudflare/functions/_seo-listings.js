...
export async function visibleSeoListingRows(db,rows,{list}={}){
 ...
 // D1 charges a row read for each attempted index lookup, even when no
 // override exists. A 100-book page previously performed up to 300 empty
 // lookups on every public request. Read the sparse table freshly instead;
 // once it exceeds the cap, fall back to exact indexed lookups so the scan
 // never becomes unbounded. Neither path caches permission/withdrawal state.
 const columns='book_id,title,author,category,visibility,logically_deleted_at,updated_at,revision'
 const sparse=await db.prepare(`SELECT ${columns} FROM central_book_overrides LIMIT 65`).all()
 let result=sparse
 if((sparse.results??[]).length>64){
  const keys=aliases.map((_,index)=>`json_extract(?1,'$[${index}]')`).join(',')
  result=await db.prepare(`SELECT ${columns} FROM central_book_overrides WHERE book_id IN (${keys})`).bind(JSON.stringify(aliases)).all()
 }
