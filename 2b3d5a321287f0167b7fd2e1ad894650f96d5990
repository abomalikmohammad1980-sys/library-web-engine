...
for(const file of source.files){
 ...
 const bytes=file.path==='app/src/engine/search_store.ts'?await readFile(resolve(root,file.path)):before
 if(file.path==='app/src/engine/search_store.ts'){file.sha256=sha(bytes);file.changed=true;await writeFile(resolve(work,file.path),bytes)}
...
await build({...config,base:'/',configFile:false,plugins:[plugin(),{name:'frozen-html',transformIndexHtml:{order:'pre',handler:()=>inputHtml}},...config.plugins],worker:{...config.worker,plugins:()=>[plugin()]},build:{...config.build,outDir:resolve(work,'compiled-final'),emptyOutDir:false}})

const fresh=resolve(work,'compiled-final/assets'),assets=resolve(out,'assets')
...
const html=await readFile(resolve(work,'compiled-final/index.html'),'utf8');await writeFile(resolve(out,'index.html'),html)
...
stage.changed.push('app/src/engine/search_store.ts');stage.payloadFingerprint=payload.fingerprint;stage.deployFingerprint=final.fingerprint;stage.files=final.fileCount
