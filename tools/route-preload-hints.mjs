...
export function inlineRoutePreloads(index,headers,hints){
 for(const [key,paths] of Object.entries(hints))if(!/^[a-z]+$/.test(key)||!Array.isArray(paths)||paths.length>80||paths.some(x=>!/^\/assets\/[A-Za-z0-9_.-]+\.js$/.test(x)))throw Error('invalid_route_preload_hints')
 const script=`(()=>{const h=${JSON.stringify(hints)};const p=(location.hash.startsWith('#/')?location.hash.slice(1):location.pathname).split('?')[0];const k=p.split('/')[1]||'home';const r=({authors:'library',author:'library',people:'library',books:'reader',book:'reader'})[k]||k;for(const href of h[r]||[]){if(document.querySelector('link[rel="modulepreload"][href="'+href+'"]'))continue;const l=document.createElement('link');l.rel='modulepreload';l.crossOrigin='';l.href=href;document.head.append(l)}})();`
