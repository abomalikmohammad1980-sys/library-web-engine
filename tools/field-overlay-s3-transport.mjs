...
export function createFieldS3Transport({ endpoint, credentials, signal, requestTimeoutMs = 15000, runTimeoutMs = 1800000, testLoopback = false }) {
  ...
  const checkKey = key => { if (typeof key !== 'string' || !key.startsWith(PREFIX) || !/^(books\/\d+\.json|manifest\.json)$/.test(key.slice(PREFIX.length))) throw error('s3_key'); };
  ...
  async function put(key, input, condition) {
    ...
    try { await operation(async abortSignal => { await sdk.send(new PutObjectCommand({ Bucket: 'khzanah-library', Key: key, Body: body, ContentLength: body.length, ContentType: 'application/json; charset=utf-8', CacheControl: 'public, max-age=31536000, immutable', IfNoneMatch: '*' }), { abortSignal }); }); }
