...
const rows=origin?manifest.filter(r=>r.path.startsWith('assets/')||['sw.js','data/shamela-search-v2-packed.js'].includes(r.path)):manifest
let cursor=0
...
// The unchanged SEO middleware rewrites HTML metadata; fingerprint static
// assets exactly and verify the frozen entry references in transformed HTML.
if(origin){const local=await readFile(root+'/deploy/pages-dist/index.html','utf8'),response=await fetch(origin+'/',{signal:AbortSignal.timeout(30000)});assert.equal(response.status,200);const html=await response.text();const refs=[...local.matchAll(/(?:src|href)="(\/assets\/[^"\s]+)"/g)].map(m=>m[1]);assert(refs.length>0);for(const ref of refs)assert(html.includes(ref),ref)}
