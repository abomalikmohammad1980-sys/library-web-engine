...
const batch=process.argv[2];if(!['51','52','53','54','55','56'].includes(batch))throw Error('reviewed_batch_required')
const base='.artifacts/batch'+(batch==='56'?'55':batch==='55'?'54':'49'),out='.artifacts/batch'+batch,app=batch==='56'?'.artifacts/performance-20260918/client-pass3':'.artifacts/batch'+(batch==='54'?'53':batch)+'-final-app',sha=b=>createHash('sha256').update(b).digest('hex')
...
if(['55','56'].includes(batch)){const receipt=JSON.parse(await readFile(base+'/deployment.json'));if(receipt.deploymentId!==current.deploymentId)throw Error('baseline_changed');stage.baselineDeploymentId=current.deploymentId}
