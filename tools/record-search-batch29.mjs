...
const root=resolve(import.meta.dirname,'..'),work=resolve(root,'.artifacts/batch29'),ops=resolve(root,'alpha-publish/ops')
...
const receipt={...stage,published:true,deploymentId:deployed.id,url:deployed.url,productionUrl:'https://khzanah.com',publishedDirectory:'../.artifacts/batch29/deploy/pages-dist',functionsSnapshot:'../.artifacts/batch29/deploy/functions',sourceSnapshot:'../.artifacts/batch29/source-snapshot.json',previousDeploymentId:previous.deploymentId,rollbackDirectory:previous.publishedDirectory,rollbackFunctionsSnapshot:'../.artifacts/batch28/deploy/functions',primaryAssetVerification:'../.artifacts/batch29/production-http.json',publishedAt:new Date().toISOString(),fullAcceptanceClosed:false,caveats:['Cold sub-second search target remains open.','A legacy normalizer source-hash test still fails, documented before this repair.']}
