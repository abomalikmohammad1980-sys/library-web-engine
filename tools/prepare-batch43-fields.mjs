...
const safeSearch=['--prepare-safe-search','--prepare-safe-search45','--prepare-safe-search46','--prepare-safe-search47','--prepare-safe-search48'].includes(process.argv[2]),batch=process.argv[2]==='--prepare-safe-search48'?'48':process.argv[2]==='--prepare-safe-search47'?'47':process.argv[2]==='--prepare-safe-search46'?'46':process.argv[2]==='--prepare-safe-search45'?'45':safeSearch?'44':'43'
const root=resolve(import.meta.dirname,'..'),base=resolve(root,'.artifacts/batch41'),app=resolve(root,`.artifacts/batch${batch==='48'?'47':batch}-final-app`),out=resolve(root,`.artifacts/batch${batch}`)
...
if(['46','47','48'].includes(batch)){
 const receipt=JSON.parse(await readFile(resolve(root,'.artifacts/snippet-recovery-publish/receipt.json'),'utf8'))
 if(!receipt.complete||receipt.checked!==944)throw Error('recovery_upload_required')
 const recovery=await snippetRecoveryConfig('https://khzanah.com')
 const configScript=`\nglobalThis.__SHAMELA_SEARCH_V2_PACKED__=Object.freeze({...globalThis.__SHAMELA_SEARCH_V2_PACKED__,batchRequests:${['47','48'].includes(batch)},sourceRecovery:${JSON.stringify(recovery).replaceAll('"https://khzanah.com/','location.origin+"/')}});\n`
 replacements.set('data/shamela-search-v2-packed.js',Buffer.from(packed+configScript))
}
...
if(['47','48'].includes(batch)){
 const bytes=await readFile(resolve(root,'server/search-batch.js')),path='deploy/functions/api/search/batch.js',target=resolve(out,path)
 await mkdir(dirname(target),{recursive:true});await writeFile(target,bytes,{flag:'wx'});sources.push({path,sha256:sha(bytes)});seen.add(path)
}
for(const entry of sources){if(seen.has(entry.path))continue;seen.add(entry.path);let bytes=await readFile(resolve(base,entry.path));if(sha(bytes)!==entry.sha256)throw Error('source_closure_changed');if(batch==='48'&&entry.path==='deploy/functions/_middleware.js'){bytes=Buffer.from(preserveSearchBatchEncoding(bytes.toString()));entry.sha256=sha(bytes)}const target=resolve(out,entry.path);await mkdir(dirname(target),{recursive:true});await writeFile(target,bytes,{flag:'wx'})}
...
const result={version:`batch-20260918-${batch}`,baselineDeploymentId:current.deploymentId,files:next.length,payloadFingerprint,boundaryPin:PIN,sourcePin,fieldsActivated:!safeSearch,functionsUnchanged:!['47','48'].includes(batch),productionReady:false,published:false}
