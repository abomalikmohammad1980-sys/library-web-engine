...
const safeSearch=['--prepare-safe-search','--prepare-safe-search45','--prepare-safe-search46','--prepare-safe-search47'].includes(process.argv[2]),batch=process.argv[2]==='--prepare-safe-search47'?'47':process.argv[2]==='--prepare-safe-search46'?'46':process.argv[2]==='--prepare-safe-search45'?'45':safeSearch?'44':'43'
...
if(['46','47'].includes(batch)){
 ...
 const configScript=`\nglobalThis.__SHAMELA_SEARCH_V2_PACKED__=Object.freeze({...globalThis.__SHAMELA_SEARCH_V2_PACKED__,batchRequests:${batch==='47'},sourceRecovery:${JSON.stringify(recovery).replaceAll('"https://khzanah.com/','location.origin+"/')}});\n`
...
if(batch==='47'){
 const bytes=await readFile(resolve(root,'alpha-publish/functions/api/search/batch.js')),path='deploy/functions/api/search/batch.js',target=resolve(out,path)
...
const result={version:`batch-20260918-${batch}`,baselineDeploymentId:current.deploymentId,files:next.length,payloadFingerprint,boundaryPin:PIN,sourcePin,fieldsActivated:!safeSearch,functionsUnchanged:batch!=='47',productionReady:false,published:false}
