...
export async function runFieldUpload(args,{planLocal=planFieldOverlay,log=console.log}={}){
 ...
 try{
  ...
  let freshState={};if(phase==='fresh_verify')try{if((await stat(`${STATE}/fresh-verification.json`)).size>8000000)throw Error('fresh_state_size');freshState=JSON.parse(await readFile(`${STATE}/fresh-verification.json`,'utf8'));if(!freshState||Array.isArray(freshState)||typeof freshState!=='object')throw Error('fresh_state_invalid')}catch(error){if(error.code!=='ENOENT')throw error}
  const result=await transferFieldOverlay({plan,journal,transport,readLocal:async job=>{if((await stat(job.localPath)).size!==job.bytes)throw Error('local_sha');return readFile(job.localPath)},persist:value=>journalWriter(`${STATE}/journal.json`,value),signal:controller.signal,maxObjects,maxBytes,freshVerify:options['--fresh-verify']==='true',freshState,persistFresh:value=>journalWriter(`${STATE}/fresh-verification.json`,value)})
