...
const root=resolve(import.meta.dirname,'..'),base=resolve(root,'.artifacts/batch76'),out=resolve(root,'.artifacts/batch77'),app=resolve(root,'.artifacts/account-recovery-20260921/client')
...
replacements.set('sw.js',Buffer.from(stampServiceWorkerRelease(sw.replace("const CACHE = 'alkhizana-shell-v18'","const CACHE = 'alkhizana-shell-account77'"),optimized.index)))
...
const changed='deploy/functions/api/account/readiness.js',seen=new Set()
for(const row of snapshot.files){const bytes=await readFile(resolve(root,row.path===changed?'deployment/cloudflare/functions/api/account/readiness.js':'.artifacts/batch76/'+row.path));if(row.path!==changed&&sha(bytes)!==row.sha256)throw Error('source_drift');if(row.path===changed){row.sha256=sha(bytes);row.bytes=bytes.length}if(seen.has(row.path))continue;seen.add(row.path);const dest=resolve(out,row.path);await mkdir(dirname(dest),{recursive:true});await writeFile(dest,bytes,{flag:'wx'})}
...
for(const [name,data] of [['static-source-manifest.json',next],['source-snapshot.json',snapshot],['deploy/wrangler.jsonc',config],['rollback.json',await json(resolve(root,'alpha-publish/ops/current-production.json'))],['stage.json',{...stage,version:'batch-20260921-77',baselineDeploymentId:'8f8603bf-31ca-48f6-acac-532ade2689f3',files:next.length,payloadFingerprint,functionsFingerprint,functionsUnchanged:false,productionReady:false,published:false,reason:'Account quota diagnostic/backoff and bounded schema check; all verified reader info actions; authentication security unchanged'}]])await writeFile(resolve(out,name),JSON.stringify(data,null,2),{flag:'wx'})
