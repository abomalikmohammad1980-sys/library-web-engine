...
const root=resolve(import.meta.dirname,'..'),base=resolve(root,'.artifacts/batch77'),out=resolve(root,'.artifacts/batch80'),app=resolve(root,'.artifacts/reader-speed-20260921/client-pass3')
...
replacements.set('sw.js',Buffer.from(stampServiceWorkerRelease(sw.replace("const CACHE = 'alkhizana-shell-v18'","const CACHE = 'alkhizana-shell-reader80'"),optimized.index)))
...
for(const [name,data] of [['static-source-manifest.json',next],['source-snapshot.json',snapshot],['deploy/wrangler.jsonc',config],['rollback.json',await json(resolve(root,'alpha-publish/ops/current-production.json'))],['stage.json',{...stage,version:'batch-20260921-80',baselineDeploymentId:'1df777d7-7a7b-4d0d-827d-ecab9e747e2b',files:next.length,payloadFingerprint,functionsFingerprint,functionsUnchanged:false,productionReady:false,published:false,reason:'Reader-safe background indexing; preserves native text flow, compact card and verified batch79 fixes'}]])await writeFile(resolve(out,name),JSON.stringify(data,null,2),{flag:'wx'})
